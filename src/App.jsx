...
function App() {
  ...
  const [boardFavorites, setBoardFavorites] = useState(() => {
    try { return new Set(JSON.parse(localStorage.getItem('keyflow_board_favorites') || '[]')) } catch { return new Set() }
  })
  ...
  const filteredBoardActivities = useMemo(() => {
    ...
    if (boardFavFilter) list = list.filter(item => boardFavorites.has(item.id))
    ...
    return list
  }, [activities, boardSearch, boardSort, boardFavFilter, boardFavorites, applications])
  ...
  useEffect(() => { localStorage.setItem('keyflow_board_favorites', JSON.stringify([...boardFavorites])) }, [boardFavorites])
