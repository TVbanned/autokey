...
function App() {
  ...
  const loadData = async () => {
    ...
    // ponytail: limit(1000) per table — admin dashboard doesn't need infinite scroll on every table;
    // if a table outgrows this, add server-side pagination on that specific tab.
    const [activityResult, applicationResult, deliveryResult, dailySubmissionResult, keyResult, invitationResult, answererResult, inboxResult, resetResult] = await Promise.all([
      supabase.from('keyflow_activities').select('*').order('created_at', { ascending: false }).limit(1000),
      supabase.from('keyflow_applications').select('*, keyflow_keys(claimed_at), keyflow_deliveries(id, status, article_url, article_title)').order('submitted_at', { ascending: false }).limit(1000),
      supabase.from('keyflow_deliveries').select('*').limit(1000),
      supabase.from('keyflow_daily_submissions').select('*').order('submitted_at', { ascending: false }).limit(1000),
      ...
      supabase.from('keyflow_invitation_codes').select('*').order('created_at', { ascending: false }).order('id').limit(1000),
      supabase.from('keyflow_answerers').select('*').order('created_at', { ascending: false }).limit(1000),
      supabase.from('keyflow_inbox').select('*').order('created_at', { ascending: false }).limit(1000),
      supabase.from('keyflow_password_reset_requests').select('*').order('requested_at', { ascending: false }).limit(1000),
    ])
    const failure = activityResult.error || applicationResult.error || deliveryResult.error || dailySubmissionResult.error || keyResult.error || invitationResult.error || answererResult.error || inboxResult.error || resetResult.error
    if (failure) setError(failure.message)
      ...
      setActivities(updated); setApplications(apps); setDeliveries(deliveryResult.data || []); setDailySubmissions(dailySubmissionResult.data || []); setKeys(keyResult.data || []); setInvitationCodes(invitationResult.data || []); setAnswerers(answererResult.data || []); setInboxMessages(inboxResult.data || []); setPasswordResetRequests(resetResult.data || [])
  ...
  const nav = [['活动看板', 'calendar'], ['活动概览', 'grid'], ['答主报名', 'users'], ['Key 管理', 'key'], ['交付验收', 'file'], ['答主管理', 'ticket'], ['合作方管理', 'users'], ['答主日常投稿', 'file'], ['剩余KEY管理', 'key'], ['页面编辑', 'image']]
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="content">
        <div className="page-title"><div><p className="eyebrow">真实数据工作台</p><h1>{active}{active === '活动看板' && <span className="board-game-count"> 当前已有 <b>{activities.length}</b> 款游戏入库</span>}{active !== '活动看板' && active !== '答主管理' && active !== '合作方管理' && active !== '答主日常投稿' && active !== '剩余KEY管理' && active !== '页面编辑' && active !== '收件箱' && selectedActivity?.game_name && <><span className="title-divider">|</span>{selectedActivity.game_name}</>}</h1><p className="subtitle">{active === '页面编辑' ? '管理注册页面的展示资源，保存后会实时同步。' : '活动、报名、Key 与交付数据均实时保存至 Supabase。'}</p></div>{active === '答主报名' ? <div style={{ display: 'flex', gap: 'var(--sp-2)' }}><button className="outline-button" onClick={() => { navigator.clipboard.writeText(claimLink); toast('申领链接已复制，可直接发送给答主') }}>复制申领链接</button><button className="outline-button preview-claim-btn" onClick={() => window.open(claimLink, '_blank')}>预览申领页</button></div> : active === '页面编辑' ? null : active === '活动看板' ? <div style={{ display: 'flex', gap: 'var(--sp-3)', alignItems: 'center' }}><div className="partner-search-wrap" style={{ background: '#fff', minWidth: 260, gap: 'var(--sp-1)', padding: 'var(--sp-1) var(--sp-2)' }}><Icon name="search" size={14} /><input className="partner-search-input" placeholder="搜索活动名称或游戏名…" value={boardSearch} onChange={e => setBoardSearch(e.target.value)} />{boardSearch && <button className="partner-search-clear" onClick={() => setBoardSearch('')}><Icon name="close" size={14} /></button>}</div><button className="primary" onClick={() => { setActivityForm({...initialActivity, ...getDefaultDeadlines()}); setActivityModal(true); }}><Icon name="plus"/> 创建活动</button></div> : <button className="primary" onClick={() => { setActivityForm({...initialActivity, ...getDefaultDeadlines()}); setActivityModal(true); }}><Icon name="plus"/> 创建活动</button>}</div>
    ...
    {activityModal && <Modal title="创建测评活动" onClose={() => setActivityModal(false)}><form onSubmit={createActivity} className="form-grid"><Field label="活动标题" required value={activityForm.title} onChange={(value) => setActivityForm({ ...activityForm, title: value })}/><Field label="游戏名称" required value={activityForm.game_name} onChange={(value) => setActivityForm({ ...activityForm, game_name: value })}/><label className="field steam-field"><span>Steam 商店地址</span><div className="steam-input-row"><input type="url" placeholder="https://store.steampowered.com/app/..." value={activityForm.steam_url || ''} onChange={(event) => setActivityForm({ ...activityForm, steam_url: event.target.value })}/><button type="button" className="btn-secondary steam-fetch-btn" onClick={handleSteamFetch} disabled={steamFetching}>{steamFetching ? '抓取中…' : '抓取'}</button></div></label><Field label="目标答主数" type="number" required value={activityForm.target_authors} onChange={(value) => setActivityForm({ ...activityForm, target_authors: value })}/><DateTimeField label="报名截止时间" value={activityForm.application_deadline} onChange={(value) => setActivityForm({ ...activityForm, application_deadline: value })}/><DateTimeField label="交付截止时间" value={activityForm.delivery_deadline} onChange={(value) => setActivityForm({ ...activityForm, delivery_deadline: value })}/><Field label="游戏简介" textarea value={activityForm.description || ''} onChange={(value) => setActivityForm({ ...activityForm, description: value })}/><Field label="测评要求" textarea value={activityForm.review_requirement || '测评要求：图文并茂，主观视角，生动有趣！'} onChange={(value) => setActivityForm({ ...activityForm, review_requirement: value })}/><PlatformSelector value={activityForm.platforms} onChange={(platforms) => setActivityForm({ ...activityForm, platforms })}/>{activityForm.game_cover && <div className="steam-preview"><img src={activityForm.game_cover} alt="封面预览"/><span>已抓取游戏封面</span></div>}<button className="primary form-submit">保存并创建</button></form></Modal>}
    ...
    {editActivityModal && <Modal title="编辑活动" onClose={() => setEditActivityModal(false)}><form onSubmit={updateActivity} className="form-grid"><Field label="活动标题" required value={activityForm.title} onChange={(value) => setActivityForm({ ...activityForm, title: value })}/><Field label="游戏名称" required value={activityForm.game_name} onChange={(value) => setActivityForm({ ...activityForm, game_name: value })}/><label className="field steam-field"><span>Steam 商店地址</span><div className="steam-input-row"><input type="url" placeholder="https://store.steampowered.com/app/..." value={activityForm.steam_url || ''} onChange={(event) => setActivityForm({ ...activityForm, steam_url: event.target.value })}/><button type="button" className="btn-secondary steam-fetch-btn" onClick={handleSteamFetch} disabled={steamFetching}>{steamFetching ? '抓取中…' : '抓取'}</button></div></label><label className="field"><span>关联合作方</span><select value={activityForm.partner_answerer_id || ''} onChange={(e) => setActivityForm({ ...activityForm, partner_answerer_id: e.target.value || null })}><option value="">— 不关联合作方 —</option>{partnerAnswerers.map((a) => <option key={a.id} value={a.id}>{a.zhihu_name}{a.wechat_id ? ` (${a.wechat_id})` : ''}</option>)}</select><small style={{color:'var(--c-ink-3)',fontSize:'var(--fs-label)',marginTop:'4px'}}>关联后，该合作方登录可查看此活动协作页。需先在「合作方管理」中生成并注册合作方账号。</small></label><Field label="目标答主数" type="number" required value={activityForm.target_authors} onChange={(value) => setActivityForm({ ...activityForm, target_authors: value })}/><DateTimeField label="报名截止时间" value={activityForm.application_deadline} onChange={(value) => setActivityForm({ ...activityForm, application_deadline: value })}/><DateTimeField label="交付截止时间" value={activityForm.delivery_deadline} onChange={(value) => setActivityForm({ ...activityForm, delivery_deadline: value })}/><Field label="游戏简介" textarea value={activityForm.description || ''} onChange={(value) => setActivityForm({ ...activityForm, description: value })}/><Field label="测评要求" textarea value={activityForm.review_requirement || '测评要求：图文并茂，主观视角，生动有趣！'} onChange={(value) => setActivityForm({ ...activityForm, review_requirement: value })}/><PlatformSelector value={activityForm.platforms} onChange={(platforms) => setActivityForm({ ...activityForm, platforms })}/>{activityForm.game_cover && <div className="steam-preview"><img src={activityForm.game_cover} alt="封面预览"/><span>已抓取游戏封面</span></div>}<button className="primary form-submit">保存修改</button></form></Modal>}
...
function ApplicationsPage({ activity, applications, answerers, authorStats, statusLabel, onSelectActivity, onAddApplication, onReviewApplication, onDeleteApplication, toast, participationByAnswerer, onViewAnswererParticipation }) {
  ...
  const downloadExcel = () => {
    const headers = ['答主', '知乎主页', '微信名', '预计字数', '报名时间', '活动参与次数', '成功完成次数', '完成率', '延迟提交', '合作方推荐', '状态']
    const rows = visibleApplications.map((p) => {
      ...
      return [p.zhihu_name, p.profile_url, p.wechat_name, `${p.expected_word_count}`, new Intl.DateTimeFormat('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }).format(new Date(p.submitted_at)), `${selectedRecords.length}`, `${completed}`, rate, `${p.delayed_count}`, p.partner_recommended ? '推荐' : '—', statusLabel[p.status]]
    })
...
function DeliveriesPage({ activity, deliveries, applications, answerers, statusLabel, notes, onNoteChange, onReview, onSelectActivity, pendingCount, approvedCount, revisionCount, toast, participationByAnswerer, onViewAnswererParticipation }) {
  ...
  const visibleDeliveries = useMemo(() => deliveryWithAuthor.filter((item) => (statusFilter === 'all' || item.status === statusFilter) && `${item.zhihu_name} ${item.article_title} ${item.article_url}`.toLowerCase().includes(keyword.trim().toLowerCase())), [deliveryWithAuthor, keyword, statusFilter])
...
function AnswererDashboard() {
  ...
  const submitDaily = async (e) => {
    ...
    const { data: activities, error: activitiesErr } = await supabase.from('keyflow_activities').select('game_name')
        ...
        {completedActivities.length ? completedActivities.map((item) => {
          ...
          return <article className="answerer-detail-item" key={item.activity_id}>
            <div className="answerer-detail-item-header">
              <div className="answerer-detail-item-cover">{item.game_cover ? <img src={item.game_cover} alt={item.game_name}/> : <span>{item.game_name?.[0] || '游'}</span>}</div>
...
function HomePage() {
  ...
  useEffect(() => {
    supabase.from('keyflow_activities')
      .select('id, title, game_name, game_cover, description, status, created_at, application_deadline, delivery_deadline, target_authors, steam_url')
      .eq('is_online', true)
      .order('created_at', { ascending: false })
      .then(({ data, error }) => {
        if (!error && data) {
          const rank = { recruiting: 0, key_distribution: 1, delivery: 2, completed: 3 }
  ...
  const uploadAvatar = async () => {
    ...
    reader.onload = async (e) => {
      const avatarUrl = e.target.result
      const { error: updateErr } = await supabase.from('keyflow_answerers').update({ avatar_url: avatarUrl, updated_at: new Date().toISOString() }).eq('id', user.id)
      if (updateErr) { setAvatarUploading(false); return }
  ...
  return <div className="home-page">
    ...
    <div className="home-main">
      <header className="home-header">
        <div className="home-brand"><span className="brand-mark zhihu-mark home-brand-mark">知</span><span>GameJourney</span></div>
        ...
        {loading ? <div className="home-games-loading"><div className="home-loading-spinner" />正在加载游戏数据…</div> : activities.length === 0 ? <div className="home-games-empty"><div className="home-empty-icon"><Icon name="grid" size={28} /></div><p>暂无入库游戏</p></div> : (
          <div className="home-game-grid">
            {activities.map((item, i) => (
              <div className="home-game-card" key={item.id} style={{ animationDelay: `${i * 0.06}s` }} onClick={() => handleCardClick(item.id)}>
                <div className="home-game-cover">
                  {item.game_cover ? <img src={item.game_cover} alt={item.game_name} /> : <span className="home-game-cover-placeholder">{item.game_name?.[0] || '?'}</span>}
...
function PartnerManagement({ codes, answerers, activities, onAddCodes, onRefresh, participationByAnswerer, onViewAnswererParticipation }) {
  ...
  return <div className="answerer-mgmt">
    ...
    <section className="panel"><div className="panel-head"><div><h3>注册合作方列表</h3><p>所有通过合作方邀请码注册的用户。</p></div><button className="outline-button compact" onClick={() => {
      ...
      URL.revokeObjectURL(url)
      toast('合作方列表已下载')
    }}>下载 Excel</button></div><div className="table-wrap"><table><thead><tr><th>#</th><th>知乎用户名</th><th>知乎主页地址</th><th>微信号</th><th>注册时间</th><th>操作</th></tr></thead><tbody>{partners.length ? partners.map((partner) => { const linkedCount = activities.filter(a => a.partner_answerer_id === partner.id).length; return <tr key={partner.id}><td><span className="serial-number">{partner.serial_number != null ? String(partner.serial_number).padStart(3, '0') : '—'}</span></td><td><div className="person">{partner.avatar_url ? <img className="person-avatar-img" src={partner.avatar_url} alt="" onClick={() => onViewAnswererParticipation(partner)} title="查看活动参与记录" style={{cursor:'pointer'}} /> : <span className="person-avatar" onClick={() => onViewAnswererParticipation(partner)} title="查看活动参与记录" style={{cursor:'pointer'}}>{partner.zhihu_name[0]}</span>}<div><strong>{partner.zhihu_name}</strong><small>合作方</small></div></div></td><td>{partner.account_address ? <a className="profile-link" href={partner.account_address} target="_blank" rel="noreferrer">查看主页</a> : '未填写'}</td><td>{partner.wechat_id || '未填写'}</td><td>{new Intl.DateTimeFormat('zh-CN', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(partner.created_at))}</td><td><button className="outline-button compact" onClick={() => setManagePartner(partner)}>管理活动{linkedCount > 0 ? ` (${linkedCount})` : ''}</button></td></tr> }) : <tr><td colSpan="6" className="table-empty">暂无注册合作方。</td></tr>}</tbody></table></div></section>
...
function AnswererManagement({ codes, answerers, activities, applications, deliveries, dailySubmissions, onAddCodes, onDeleteAnswerer, participationByAnswerer, onViewAnswererParticipation }) {
  const [generating, setGenerating] = useState(false)
  const [notice, setNotice] = useState('')
  ...
  return <div className="answerer-mgmt">
    ...
    <section className="panel">
      ...
      <div className="table-wrap"><table><thead><tr><th>#</th><th>知乎用户名</th><th>知乎主页地址</th><th>微信号</th><th>活动参与次数</th><th>成功完成次数</th><th>完成率</th><th>延迟提交</th><th>上次提交作品</th><th>参与记录</th><th>注册时间</th><th>操作</th></tr></thead><tbody>{filteredAnswerers.length ? filteredAnswerers.map((a) => {
        const records = participationByAnswerer[a.id] || []
        const selectedRecords = records.filter((r) => r.status === 'selected')
        const completed = records.filter((r) => (r.all_deliveries || []).some(d => d.article_url)).length
        const rate = selectedRecords.length ? `${Math.round(completed / selectedRecords.length * 100)}%` : '—'
        const delayedTotal = delayedByAnswerer[a.id] || 0
        return <tr key={a.id}><td><span className="serial-number">{a.serial_number != null ? String(a.serial_number).padStart(3, '0') : '—'}</span></td><td><div className="person">{a.avatar_url ? <img className="person-avatar-img" src={a.avatar_url} alt="" onClick={() => onViewAnswererParticipation(a)} title="查看活动参与记录" style={{cursor:'pointer'}} /> : <span className="person-avatar" onClick={() => onViewAnswererParticipation(a)} title="查看活动参与记录" style={{cursor:'pointer'}}>{a.zhihu_name[0]}</span>}<div><strong>{a.zhihu_name}</strong><small>答主</small></div></div></td><td>{a.account_address ? <a className="profile-link" href={a.account_address} target="_blank" rel="noreferrer">查看主页</a> : <span style={{ color: 'var(--c-ink-4)', fontSize: 'var(--fs-meta)' }}>未填写</span>}</td><td>{a.wechat_id || '未填写'}</td><td>{selectedRecords.length}</td><td>{completed}</td><td>{rate}</td><td><span className={`history-count ${delayedTotal > 0 ? 'highlight-red' : ''}`}>{delayedTotal} <small>次</small></span></td><td>{formatLastSubmission(lastSubmissionByAnswerer[a.id])}</td><td><button className="outline-button compact" onClick={() => onViewAnswererParticipation(a)}>查看记录</button></td><td>{new Intl.DateTimeFormat('zh-CN', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(a.created_at))}</td><td><button className="delete-action" onClick={() => setConfirmDeleteId(a.id)}>删除</button></td></tr>
      }) : <tr><td colSpan="12" className="table-empty">暂无注册答主。</td></tr>}</tbody></table></div>
    </section>
...
function DailySubmissionsCalendar({ submissions, selectedDate, onSelectDate }) {
  ...
  const submissionByDate = useMemo(() => {
    ...
    submissions.forEach(s => {
      ...
      if (!map[key]) map[key] = { total: 0, unreviewed: 0 }
      map[key].total++
      if (!s.reviewed) map[key].unreviewed++
    })
    return map
  }, [submissions])
    ...
    <div className="daily-calendar-grid">
      ...
      {cells.map((cell, i) => <div key={i} className={`daily-calendar-cell${!cell ? ' empty' : ''}${cell && cell.key === selectedDate ? ' selected' : ''}${cell && cell.key === today ? ' today' : ''}`} onClick={() => cell && onSelectDate(cell.key === selectedDate ? null : cell.key)}>
        {cell && <>
          ...
          {cell.info && <span className={`daily-calendar-dot${cell.info.unreviewed > 0 ? ' unreviewed' : ''}`}>{cell.info.total}</span>}
        </>}
      </div>)}
    </div>
...
function DailySubmissionsPage({ submissions, answerers, toast, setDailySubmissions, participationByAnswerer, onViewAnswererParticipation }) {
  ...
  const handleProcess = async (submission) => {
    const { data: existing } = await supabase.from('keyflow_inbox').select('id').eq('to_id', submission.answerer_id).eq('type', 'private_message').eq('data->>submission_id', String(submission.id)).maybeSingle()
    if (!existing) {
    ...
    setDailySubmissions(prev => prev.map(s => s.id === submission.id ? { ...s, processed: true } : s))
    supabase.from('keyflow_daily_submissions').update({ processed: true }).eq('id', submission.id).then(() => {})
  }
  ...
  const handleBatchProcess = async () => {
    ...
    for (const id of ids) {
      ...
      const { data: existing } = await supabase.from('keyflow_inbox').select('id').eq('to_id', s.answerer_id).eq('type', 'private_message').eq('data->>submission_id', String(s.id)).maybeSingle()
      if (!existing) {
        await supabase.from('keyflow_inbox').insert({
      ...
      setDailySubmissions(prev => prev.map(sub => sub.id === id ? { ...sub, processed: true } : sub))
      supabase.from('keyflow_daily_submissions').update({ processed: true }).eq('id', id).then(() => {})
    }
