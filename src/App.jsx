...
function App() {
  ...
  const loadData = async () => {
    ...
    const [activityResult, applicationResult, deliveryResult, dailySubmissionResult, keyResult, invitationResult, answererResult, inboxResult, resetResult] = await Promise.all([
      ...
      supabase.from('keyflow_applications').select('*, keyflow_keys(claimed_at), keyflow_deliveries(id, status, article_url, article_title)').order('submitted_at', { ascending: false }),
      supabase.from('keyflow_deliveries').select('*'),
      supabase.from('keyflow_daily_submissions').select('answerer_id, submitted_at'),
    ...
    if (requestError) {
      if (requestError.code === '23505') {
        const { data: existing } = await supabase.from('keyflow_deliveries')
          .select('id, status, article_url, article_title').eq('application_id', application.id).single()
...
function AnswererManagement({ codes, answerers, activities, applications, deliveries, dailySubmissions, onAddCodes, onDeleteAnswerer }) {
  ...
  return <div className="answerer-mgmt">
    ...
    <section className="panel">
      <div className="panel-head"><div><h3>注册答主列表</h3><p>所有通过邀请码注册的答主账号。</p></div><div style={{ display: 'flex', gap: 'var(--sp-3)', alignItems: 'center' }}><div className="partner-search-wrap" style={{ background: '#fff', minWidth: 220, gap: 'var(--sp-1)', padding: 'var(--sp-1) var(--sp-2)' }}><Icon name="search" size={14} /><input className="partner-search-input" placeholder="搜索答主名称、微信或主页…" value={search} onChange={e => setSearch(e.target.value)} />{search && <button className="partner-search-clear" onClick={() => setSearch('')}><Icon name="close" size={14} /></button>}</div><button className="outline-button compact" onClick={() => {
        const headers = ['注册用户ID', '知乎用户名', '知乎主页地址', '微信号', '活动参与次数', '成功完成次数', '完成率', '上次提交作品', '注册时间']
        const rows = answerers.map((a) => {
          ...
          return [a.serial_number != null ? String(a.serial_number).padStart(3, '0') : '—', a.zhihu_name, a.account_address || '未填写', a.wechat_id || '未填写', `${selectedRecords.length}`, `${completed}`, rate, formatLastSubmission(lastSubmissionByAnswerer[a.id]), new Intl.DateTimeFormat('zh-CN', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(a.created_at))]
        })
        const csv = '\uFEFF' + [headers, ...rows].map((row) => row.map((cell) => `"${String(cell).replace(/"/g, '""')}"`).join(',')).join('\n')
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' })
        const url = URL.createObjectURL(blob)
