-- ponytail: make keyflow_claim_key idempotent — return existing key if already claimed
-- prevents double-claim when user revisits the claim page without localStorage
...
declare
  ...
  v_existing_id uuid;
begin
  -- return existing key if this application already claimed one
  select k.id into v_existing_id from keyflow_keys k where k.application_id = p_application_id limit 1;
  if v_existing_id is not null then
    return query select k.key_value, k.claimed_at from keyflow_keys k where k.id = v_existing_id;
    return;
  end if;
