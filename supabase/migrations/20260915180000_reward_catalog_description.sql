...
begin
  ...
  insert into public.keyflow_reward_catalog (title, description, image_url, category, card_tags, cost_coins, reimbursement_discount, min_level, stock_total, stock_left, fulfillment_type, status)
  values (trim(p_product->>'title'), left(trim(coalesce(p_product->>'description', '')), 80), coalesce(p_product->>'image_url', ''), v_category, trim(coalesce(p_product->>'card_tags', '')),
...
begin
  ...
  update public.keyflow_reward_catalog set
    title = trim(p_product->>'title'), description = left(trim(coalesce(p_product->>'description', '')), 80), image_url = coalesce(p_product->>'image_url', ''), category = v_category, card_tags = trim(coalesce(p_product->>'card_tags', '')),
