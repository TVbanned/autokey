-- 看板只返回最近的作品，避免投稿后全量聚合与渲染历史记录
create index if not exists keyflow_daily_submissions_answerer_created_at_idx
  on public.keyflow_daily_submissions (answerer_id, created_at desc);

create index if not exists keyflow_daily_submissions_answerer_submitted_at_idx
  on public.keyflow_daily_submissions (answerer_id, submitted_at desc);
