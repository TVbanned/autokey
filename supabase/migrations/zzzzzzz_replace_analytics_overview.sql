-- 数据概览改为近 30 天 Key 领取热力图和答主页面事件。
alter table public.keyflow_page_views
  drop constraint if exists keyflow_page_views_page_type_check;

alter table public.keyflow_page_views
  add constraint keyflow_page_views_page_type_check
  check (page_type in ('home', 'claim', 'answerer_dashboard'));
