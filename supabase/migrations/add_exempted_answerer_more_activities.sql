-- 豁免答主：在 more_activities 中也显示 key_distribution 和 delivery 阶段的活动
...
begin
  ...
  return jsonb_build_object(
    ...
    'more_activities', coalesce((
      ...
      where a.is_online = true
        and (
          a.status = 'recruiting'
          or (
            a.status in ('key_distribution', 'delivery')
    ...
    'historical_activities', coalesce((
      ...
      from (
        -- 活动已完成 / 非草稿非招募中（原有逻辑 + had_delivery 兜底）
